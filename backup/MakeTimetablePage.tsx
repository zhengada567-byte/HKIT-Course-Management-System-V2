...
import {
  generateTimetablePlanningModules,
...
function isCommonStreamDisplay(streamCode: string | null | undefined) {
...
function renderModuleCodeAndName(row: {
...
function renderModuleInstanceAndName(row: {
...
async function refreshTimetableAndAssignments(filters?: {
 ...
 async function handleGeneratePlanning() {
  if (!user) {
    setMessage("Please login before generating planning modules.");
    return;
  }
  ...
  try {
    await generateTimetablePlanningModules({
      academicYear,
      programmeCode: programmeCode || undefined,
      streamCode: streamCode || undefined,
      createdBy: user.id,
    ...
    const refreshedPlanning = await listPlanningModulesWithStudentNumbers({
      academicYear,
      programmeCode: programmeCode || undefined,
      streamCode: streamCode || undefined,
    });

    setPlanningModules(refreshedPlanning);
    await refreshStudentRows(refreshedPlanning);
    ...
    setMessage(
      "Planning modules generated. Please continue with student numbers."
    );
  } catch (error) {
    console.error("[MakeTimetablePage] Generate planning failed:", error);
...
async function handleNoSplitSingle(module: TimetablePlanningModuleRow) {
  console.log("[MakeTimetablePage] handleNoSplitSingle called", {
    module,
    user,
  });
...
async function handleSplitSingle(
  ...
  console.log("[MakeTimetablePage] handleSplitSingle called", {
    module,
    numberOfClasses,
    user,
...
async function handleCreateCombinedSplit(
  ...
  console.log("[MakeTimetablePage] handleCreateCombinedSplit called", {
    group,
    numberOfClasses,
    user,
  });
...
async function handleConfirmAllSplitDecisions() {
  console.log("[MakeTimetablePage] handleConfirmAllSplitDecisions called", {
    user,
  });
  ...
  try {
    ...
    for (const module of pendingSingleModules) {
      const student = latestStudentRows.find(
        (row) =>
          row.academic_year === module.academic_year &&
          row.module_code === module.module_code &&
          row.programme_code === module.programme_code
...
async function handleUndoTimetableModule(row: TimetableModuleRow) {
  console.log("[MakeTimetablePage] handleUndoTimetableModule called", {
    row,
    user,
  });
...
if (step === "split" || step === "assignment") {
  ...
  }, [academicYear, programmeCode, streamCode, step]);
...
function PlanningStep({
  ...
  if (planningModules.length === 0) {
    return <EmptyState message="Generate modules to start timetable planning." />;
  }
...
function StudentNumberStep({
  ...
  if (rows.length === 0) {
    return <EmptyState message="No student number rows. Generate planning modules first." />;
  }
  ...
  return (
    <div className="space-y-4">
      <DataTable
        rows={rows}
        rowKey={(row) =>
          `${row.academic_year}-${row.module_code}-${row.programme_code}`
        }
        columns={[
          {
            key: "module",
...
function SplitStep({
  ...
  <div>
    ...
    <div className="text-sm text-blue-700">
      Pending single modules: {singleModules.length}. Pending combined groups:{" "}
      {pendingManualGroups.length}. Remaining single modules will be marked as No Split.
    </div>
  </div>
  ...
  >
    Confirm All Split Decisions
      ...
      <div className="card">
        <div className="card-header font-semibold">Combined Modules Ready for Split</div>
      ...
      <div className="card">
        <div className="card-header font-semibold">Single Module for Split</div>
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (row) => {
                    const student = studentRows.find(
                      (s) =>
                        s.academic_year === row.academic_year &&
                        s.module_code === row.module_code &&
                        s.programme_code === row.programme_code
                    );
