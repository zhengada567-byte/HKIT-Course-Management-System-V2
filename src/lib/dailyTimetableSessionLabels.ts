...
/**
 * Assign L/T labels strictly by date order (no vacant holes from cancelled rows).
 * Cancelled → clear. Extra active sessions beyond the sequence → backup (null).
 */
export function buildChronologicalLabelAssignments(params: {
  labelSequence: DailySessionLabelSlot[];
  sessions: SessionLabelAssignmentTarget[];
}): SessionLabelAssignmentResult[] {
  const results: SessionLabelAssignmentResult[] = [];
  ...
  for (let index = 0; index < active.length; index += 1) {
    ...
    const slot = params.labelSequence[index];
